...
public unsafe readonly struct TextureData
{
    ...
    /// </summary>
    public int UniqueId => Native->UniqueID;
    ...
    {
        get => (TextureStatus)Native->Status;
    ...
    {
        get => ImTextureData.GetTexID(Native);
    ...
    /// </summary>
    public TextureFormat Format => (TextureFormat)Native->Format;
    ...
    /// </summary>
    public int BytesPerPixel => Native->BytesPerPixel;

    /// <summary>
    /// Gets the total size of the pixel buffer in bytes.
    /// </summary>
    public int SizeInBytes => Native->Width * Native->Height * Native->BytesPerPixel;

    /// <summary>
    /// Gets the pitch (bytes per row) of the texture.
    /// </summary>
    public int Pitch => Native->Width * Native->BytesPerPixel;
    ...
    /// </summary>
    public int UnusedFrames => Native->UnusedFrames;

    /// <summary>
    /// The number of contexts using this texture.
    /// </summary>
    public ushort RefCount => Native->RefCount;

    /// <summary>
    /// Whether the texture data uses colors (rather than just white + alpha).
    /// </summary>
    public bool UseColors => Native->UseColors;

    /// <summary>
    /// The bounding box encompassing all past and queued updates.
    /// </summary>
    public TextureRect UsedRect => new(Native->UsedRect);

    /// <summary>
    /// The bounding box encompassing all queued updates.
    /// </summary>
    public TextureRect UpdateRect => new(Native->UpdateRect);
    ...
    public Span<byte> GetPixelsAt(int x, int y)
    {
